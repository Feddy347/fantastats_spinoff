...
//
// FS Rate viene SEMPRE ricalcolato dai Fantapunti salvati: se in futuro
// cambiamo formula, lo storico viene aggiornato automaticamente al successivo run.
...
export function updatePlayerStats({ gw, listone, statsById, flags }) {
  ...
  for (const [playerId, stats] of statsById) {
    ...
    const primaryRole =
      (rec.role_fantastats ?? '').split(';')[0]?.trim() || 'C'
  ...
  const allRows = [...kept, ...newRows].map((r) => {
    ...
    const normalRate = fsRateFromPoints(Number(r.punteggio ?? 0), false)
    const flopRate = fsRateFromPoints(Number(r.punteggio_flop ?? 0), true)
    ...
    .map(([id, a]) => ({
      ...
      precisione_passaggi:
        a.precisione_passaggi_gw > 0
          ? Number((a.precisione_passaggi_tot / a.precisione_passaggi_gw).toFixed(2))
          : 0,
